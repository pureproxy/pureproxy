...
class PureProxy extends net.Server {
    ...
    createHttpResponseParser() {
        return new HTTPParser(HTTPParser.RESPONSE)
    }
    ...
    onHttpConnectionHandler(socket) {
        ...
        requestParser[kOnHeadersComplete] = async(versionMajor, versionMinor, headers, method, uri, statusCode, statusMessage, upgrade, shouldKeepAlive) => {
            ...
            if (method === connectMethod) {
                requestParser[kOnMessageComplete] = async() => {
                    ...
                    try {
                        client = await this.getClient(hostname || this.getHttpHeader(headers, 'host'), port ? parseInt(port) : 443, context)
                    }
                    catch (e) {
                        this.emit('error', e)
                ...
                try {
                    client = await this.getClient(hostname || this.getHttpHeader(headers, 'host'), port ? parseInt(port) : 80, context)
                }
                catch (e) {
                    this.emit('error', e)
                ...
                requestParser[kOnMessageComplete] = () => {
                    delete requestParser[kOnBody]
                    delete requestParser[kOnMessageComplete]
                ...
                responseParser[kOnHeadersComplete] = async(versionMajor, versionMinor, headers, method, uri, statusCode, statusMessage, upgrade, shouldKeepAlive) => {
                    ...
                    responseParser[kOnMessageComplete] = () => {
                        delete responseParser[kOnBody]
                        delete responseParser[kOnMessageComplete]
